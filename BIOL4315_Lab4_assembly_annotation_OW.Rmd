...
group1Lst <- fileList[1:12]
group2Lst <- fileList[13:24]
group3Lst <- fileList[25:36]

rqc_results <- rqc(path = fq_path, pattern = ".fastq.gz", openBrowser = FALSE)

group1Plot <- rqcCycleQualityBoxPlot(rqc_results[1:12])
group1Plot
```
...
QuasR::preprocessReads(fastqFiles, processedFiles,
                       nBases = 1,
                       truncateEndBases = 3,
                       Lpattern = "GCCCGGGTAA")
```